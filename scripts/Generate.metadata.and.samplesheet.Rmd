...
params: 
  Assay: 
    value: "UDI"
    input: select
    choices: [Nextera XT, UDI, UDI_NextFlex]
  Index_Adapters:
    value:  "UDI 386 Samples"
    input: select
    choices: [Nextera XT Index Kit (96 Indexes 384 Samples),
    UDI 386 Samples, UDI_NextFlex]
  Match_with_well:
    value:  "Yes"
...
UDI_NextFlex <- read_csv(here("data_sub", "next_flex_barcodes.csv"))
```
...
```{r UDI Match}
if(params$Assay == "UDI" & params$Match_with_well == "Yes"){
  test <-  nrow(inner_join(init.metadata, UDI_Nextera,by = c("Well", "Set"))) == nrow(left_join(init.metadata, UDI_Nextera,by = c("Well", "Set")))
test
  
   if (test ) {
  left_join(init.metadata, UDI_Nextera, by = c("Well", "Set")) -> metadata
    } else{
    # Is it a problem with the set names?
  if (!unique(init.metadata$Set) %in% 1:4){
...
if(params$Assay == "UDI_NextFlex" ){
   ...
   case_when(nrow(anti_join(init.metadata, UDI_NextFlex, by = "UDI_Index_Name"))  == 0 ~ "All indices present in the  dataset",
          TRUE                                                     ~ "Either some of the indices are not present in the NextFlex Indices list - That will result in NAs in the final Sample Sheet")

  left_join(init.metadata, UDI_NextFlex ,by = "UDI_Index_Name" ) -> metadata
