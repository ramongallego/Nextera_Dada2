...
case_when(nrow(anti_join(init.metadata, Nextera_Adapters_i5)) + nrow(anti_join(init.metadata, Nextera_Adapters_i7)) == 0 ~ "All indices present iun the  dataset",
           # == 0 ~ "Pass i7",
...
```{r, echo=F}
metadata %>%
  mutate(Sample_name = str_replace_all(Sample_name,pattern = "[\\.|_]",replacement = "-")) %>% 
